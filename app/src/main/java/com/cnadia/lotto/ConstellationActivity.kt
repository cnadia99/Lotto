...
class ConstellationActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var constelString = makeConstellationString(datePicker.month, datePicker.dayOfMonth)

        goResultButton.setOnClickListener {
            val intent = Intent(this, ResultActivity::class.java)
            intent.putIntegerArrayListExtra("result", ArrayList(LottoNumberMaker.getLottoNumberFromHash(constelString)))
            intent.putExtra("constellation", constelString)
            startActivity(intent)
        }

        textView.text = constelString

        val calendar = Calendar.getInstance()
        datePicker.init(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH)
        ) { view, year, monthOfYear, dayOfMonth ->
            constelString = makeConstellationString(datePicker.month, datePicker.dayOfMonth)
            textView.text = constelString
        }
    }
